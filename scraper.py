...
class GallupScraper(BaseScraper):
    ...
    def scrape_page(self):
        ...
        images = self.soup.find_all("div", {"class":"sggt-image"})
        parsed_graphs = self._parse_graph_summaries(images)
        tables = self.soup.find_all("figure", {"class":"figure-table"})
        parsed_tables = self._parse_tables(tables)
        return parsed_graphs, parsed_tables
    ...
    def _parse_tables(self, tables):
        ...
        for table in tables:
            if len(table.find_all("th", {"scope" : "rowgroup"})) >= 1:
                table_dict = self._parse_table_with_groups(table)
            else:
                table_dict = self._parse_table_without_groups(table)
            table_dicts.append(table_dict)
        return table_dicts

    def _parse_graph_summaries(self, images):
        summary_dicts = []
        for image in images:
            summary = self._parse_graph_summary(image)
            summary_dicts.append(summary)
        return summary_dicts

    def _parse_graph_summary(self, image):
    ...
    def _parse_table_without_groups(self, table):
        desc = table.figcaption.text.strip()
        rows = table.tbody.find_all("tr")
        table_dict = {'description': desc}
        for row in rows:
            table_dict[row.th.text] = self._get_values_for_row(row)
        return table_dict
